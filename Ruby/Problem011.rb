# encoding: UTF-8
# Project Euler
# Solution to Problem 11 in Ruby
# Problem URL: http://projecteuler.net/problem=11
# Author: Pascal Batty
# 
# ======================================================
#
...
grid.each do |line|
	line.each_cons(count) do |group|
		prod = group.reduce(:*)
		p "#{group} => #{prod}"
		greatest = prod if prod > greatest
	end
end

rotated_grid = grid.transpose

rotated_grid.each do |line|
	line.each_cons(count) do |group|
		prod = group.reduce(:*)
		p "#{group} => #{prod}"
		greatest = prod if prod > greatest
...
0.upto(grid.size - count) do |i|
	0.upto(grid[i].size - count) do |j|
		diag = Array.new
		(0...count).each do |delta|
			diag << grid[i+delta][j+delta]
		end
		prod = diag.reduce(:*)
		p "#{diag} => #{prod}"
		greatest = prod if prod > greatest
